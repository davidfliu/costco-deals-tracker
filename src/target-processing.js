 ...
 */
import { fetchContent, parsePromotions, detectChanges, filterMaterialChanges, formatSlackMessage, sendSlackNotification, hashString } from "./utils";
import { readTargetState, writeTargetState, storeAndPruneSnapshot, readTargets } from "./kv-storage";
import { PerformanceMonitor, RequestOptimizer } from "./performance";
/**
 ...
 */
export async function processTarget(env, target) {
    ...
    try {
        ...
        if (changes.hasChanges && env.SLACK_WEBHOOK) {
            try {
                const message = formatSlackMessage(target.name || 'Costco Travel Deal', target.url, changes, currentTimestamp);
 ...
 */
export async function processBatchTargets(env) {
    const startTime = Date.now();
    ...
    try {
        ...
        const processingPromises = enabledTargets.map(target => processTarget(env, target).catch(error => ({
            target,
            success: false,
        ...
        const summary = generateBatchSummary(totalTargets, successfulTargets, failedTargets, targetsWithChanges, notificationsSent);
 ...
 * @returns Processing result with success status and change information
 */
async function processTargetOptimized(env, target, previousState, textProcessor) {
    const startTime = Date.now();
    try {
        console.log(`Processing target: ${target.name || target.url}`);
 ...
 */
function generateBatchSummary(totalTargets, successfulTargets, failedTargets, targetsWithChanges, notificationsSent) {
    const parts = [];
