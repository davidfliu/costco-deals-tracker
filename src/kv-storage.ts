 ...
 */
export function validateTargets(targets: any): targets is Target[] {
 ...
 */
export async function readTargets(env: Env): Promise<Target[]> {
  try {
 ...
 */
export async function writeTargets(env: Env, targets: Target[]): Promise<void> {
  if (!validateTargets(targets)) {
 ...
 */
export async function upsertTarget(env: Env, target: Target): Promise<void> {
 ...
 */
export async function removeTarget(env: Env, url: string): Promise<boolean> {
  const targets = await readTargets(env);
 ...
 */
export async function readTargetState(env: Env, url: string): Promise<TargetState | null> {
  try {
 ...
 */
export async function writeTargetState(env: Env, url: string, state: TargetState): Promise<void> {
  if (!validateTargetState(state)) {
 ...
 */
export function shouldUpdateState(currentState: TargetState, previousState: TargetState | null): boolean {
  if (!previousState) {
 ...
 * @throws Error if state is invalid or KV operation fails
 */
export async function updateTargetStateIfChanged(
  env: Env, 
  url: string, 
 ...
 */
export function validateTargetState(state: any): state is TargetState {
 ...
 */
function validatePromotion(promo: any): boolean {
 ...
 * @throws Error if KV operation fails
 */
export async function deleteTargetState(env: Env, url: string): Promise<void> {
  try {
    const stateKey = await generateStateKey(url);
 ...
 */
export async function pruneHistoricalSnapshots(
  env: Env, 
  url: string, 
  keepCount: number = 5
): Promise<number> {
 ...
 * @throws Error if snapshot is invalid or KV operation fails
 */
export async function storeAndPruneSnapshot(
  env: Env, 
  url: string, 
  snapshot: HistoricalSnapshot, 
  keepCount: number = 5
 ...
 */
export function validateHistoricalSnapshot(snapshot: any): snapshot is HistoricalSnapshot {
 ...
 * @returns Number of snapshots that were deleted
 */
export async function deleteAllHistoricalSnapshots(env: Env, url: string): Promise<number> {
  try {
    const urlHash = await import('./utils').then(utils => utils.hashString(url));
    const prefix = `hist:${urlHash}:`;
